        ...
        try {
            ...
            for (AssetBalance assetBalance : accountUpdateEvent.getBalances()) {
                if ("ETH".equals(assetBalance.getAsset())) {
                    assertEquals(0, valueOf(0.1).compareTo(assetBalance.getFree()));
                } else {
                    assertEquals(0, ZERO.compareTo(assetBalance.getFree()));
                }
                assertEquals(0, ZERO.compareTo(assetBalance.getLocked()));
            }
        } catch (IOException e) {
            fail();
        }
